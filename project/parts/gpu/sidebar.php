...
<!-- ########################################################################################### -->
<?php
$headers = array("Card","Brand","Price","Core Clock","Memory Type","Memory","Memory Interface");
$columns = array("gpu","brand","price","coreClock","memoryType","memory","memoryInterface");
$a=0;
while($a < sizeof($headers)){
  //get fields from database
  $query = $conn->prepare("SELECT DISTINCT $columns[$a] FROM gpumem NATURAL JOIN videoCard WHERE $columns[$a] <> 'NULL' ORDER BY $columns[$a] ASC");
  $query->execute();
  ...
  <div class="panel panel-default">
    ...
    <div id="collapse'.$a.'" class="panel-collapse collapse">
      ...
      <form action="/PC-Parts-Database/project/parts/gpu/gpu.php" method="get">';
      $b=0;
